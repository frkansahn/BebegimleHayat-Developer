<template>
    <div class="add-blog" id="addBlog">
                    ...
                    <b-form class="row">
                        <div class="col-12 mt-4">
                            <div class="row mb-4">
                                ...
                                <b-button class="col-12" type="button" @click="saveBlog" squared size="lg"
                        ...
                        <div class="col-md-9 mb-4">
                            <div class="row">
                                <b-form-group class="col-md-12" id="subject_group" label="Başlık" label-for="subject_input"
                                    ...
                                    <b-form-input :class="[blogError.subject ? 'border-danger' : 'border-dark']"
                                        id="subject_input" v-model="blog.subject" type="text" name="subject"
                                        placeholder="Başlık" @click="blogError.subject = false" @blur="convertTurkishCharacter()" required></b-form-input>
                                </b-form-group>

                                <b-form-group class="col-md-7" id="image_group" label="Makale Görseli" label-for="image_input"
                                    description="Makale kartlarında gösterilecek görseldir.">
                                ...
                                <div class="col-md-2 mt-3">
                                    <div class="row">
                                        <b-button @click="saveImage" class="w-100" variant="success" size="md" v-if="!blog.image">Resim Ekle</b-button>
                                        <b-button @click="deleteImage" class="w-100" variant="danger" size="md" v-else>Sil</b-button> 
                                    </div>
                                </div>
                                <div class="col-md-3 px-4">
                                    <img :src="blog.image" :alt="blog.subject" v-if="blog.image" style="max-height: 60px;">
                        ...
                        <b-form-group class="col-md-4" id="seo_link_group" label="Seo Link" label-for="seo_link_input" description="">                            
                            <div class="d-flex align-items-center justify-content-center">
                                <b-form-input class="float-left w-75" :state="seoLinkSuccess" :class="[seoLinkSuccess == undefined ? 'border-dark':'']" id="seo_link_input" v-model="blog.seo_link" type="text" name="seo_link" placeholder="Seo linkini giriniz" @click="blogError.seo_link=false" @blur="controlSeoLink" @keypress="keypressSeoLink($event)" required></b-form-input>
                        ...
                        <b-form-group class="col-md-3" id="sort_group" label="Sıralama" label-for="sort_input"
                            description="">
                        ...
                        <b-form-group class="col-md-12" id="short_description_group" label="Kısa Açıklama"
                            ...
                            <b-form-textarea class="border-dark"
                                id="short_description_input" v-model="blog.short_description" type="text"
                                name="short_description" placeholder="Kısa Açıklama" required></b-form-textarea>
                        </b-form-group>
                        ...
                        <div class="col-md-3 border-right">
                            <draggable :list="blog.sections">
                                <div class="col-12 border rounded mb-3 py-3 bg-secondary" role="button" v-for="(section, index) in blog.sections">
                                    <div class="row">
                                        <div class="col-12">
                                            <span class="float-right">
                                                <b-icon icon="trash" variant="danger" font-scale="1" class="mr-2" role="button" @click="deleteSection(index)"></b-icon>
                                            </span>
                                        </div>
                                        <div class="col-12 mb-2">
                                            <b-form-group class="text-white" id="section_name_group" label="Bölüm adı"
                                                ...
                                                <b-form-input class="border-dark" id="section_name_input" v-model="section.name"
                                                    type="text" :name="`section_name_${index}`" placeholder="Bölüm adı giriniz"></b-form-input>
                                            </b-form-group>
                                        </div>
                                        <div class="col-12">
                                            <button type="button" class="btn btn-sm mr-2 py-1 px-2" :class="section.type == 'content' ? 'btn-success':'btn-light'" @click="section.type = 'content'">
                                                içerik
                                            </button>
                                            <button type="button" class="btn btn-sm py-1 px-2" :class="section.type == 'product' ? 'btn-success':'btn-light'" @click="section.type = 'product'">
                                                Ürün
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div class="row">
                                <div class="col-12 mb-3 pb-3 border-bottom" v-for="(section, index) in blog.sections">
                                    <div v-if="section.type == 'content'">
                                        ...
                                        <editor v-model="section.value" :name="`editor_${index}`" api-key="2tc56hde9q30zeb835bczxj559w0h7vtwb9znqpa08y8e605" :init="init" required />
                                    </div>
        ...
        <b-modal id="blog-detail" title="Blog Detay" size="xl">
            <div class="row">
                <div class="col-12">
        ...
        data() {
            return {
                ...
                init: {
                    ...
                    toolbar: 'insertfile undo redo | styleselect | fullscreen | forecolor  | bold italic underline italic | alignleft aligncenter alignright alignjustify | outdent indent | link image media| code table | numlist bullist | ltr rtl | emoticons | wordcount | anchor | responsivefilemanager | template | preview | grid_insert',
        ...
        methods: {
            ...
            async controlSeoLink(){
                return new Promise((resolve,reject) => {
                    ...
                    {
                        _this.$repositories.blog.controlLink(_this.blog.seo_link)
                        .then(res => {
                            if(res.data.use)
            ...
            async saveBlog() {
                ...
                if (_this.seoLinkSuccess && _this.$checkIsNullOrEmpty(_this.blog.subject) && _this.$checkIsNullOrEmpty(_this.blog.category_id)) {
                    ...
                    _this.$repositories.blog.create(_this.blog)
                        .then(res => {
                            if (res.data.success == "success") {
                                ...
                                setTimeout(() => {
                                    _this.$router.push("/admin/blog-detail/" + res.data.response.insertId);
                                }, 2000)
                            }
                            else {
