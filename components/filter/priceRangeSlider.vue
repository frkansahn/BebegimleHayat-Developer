...
export default {
    ...
    watch: {
        async $route(to, from) {
            let selectedPrice = this.$route.query?.price?.split('-');
            this.minValue = parseFloat(selectedPrice && selectedPrice[0]) || this.min;
            this.maxValue = parseFloat(selectedPrice && selectedPrice[1]) || this.max;
        }
    },
